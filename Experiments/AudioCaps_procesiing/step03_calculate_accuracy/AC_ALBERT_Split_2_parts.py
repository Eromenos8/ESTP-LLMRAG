...
class AudioCapsMatcher:
    ...
    def plot_accuracy_comparison(self, all_accuracies: List[List[float]], output_dir: str):
        ...
        # set line colour and markers
        colors = plt.cm.viridis(np.linspace(0, 1, len(all_accuracies)))
        markers = ['o', 's'][:len(all_accuracies)]
    ...
    def process_single_part(self, matched_df: pd.DataFrame, train_df: pd.DataFrame, 
        ...
        # index
        self.logger.info(f"construct Part {part_id} index...")
        text_embeddings = self.encode_texts(descriptions, f"coding Part {part_id} description text")
    ...
    def process_and_match(self, train_path: str, matched_results_path: str, output_dir: str):
        ...
        # split
        splits = self.split_datasets(train_filtered, matched_filtered, num_parts=2)
        ...
        print("\nsummary:")
        print("\nPart 1:")
        for k, acc in enumerate(all_accuracies[0], 1):
...
def main():
    ...
    output_dir = "/root/autodl-tmp/AudioCaps/ALBERT_matched_result_2PARTS"
