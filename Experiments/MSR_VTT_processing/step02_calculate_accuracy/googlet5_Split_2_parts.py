...
class MultiPartT5Analyzer:
    ...
    def split_datasets(self, df: pd.DataFrame, video_text_map: Dict[str, str], 
                      num_parts: int = 2, random_state: int = 42):
    ...
    def plot_multipart_comparison(self, all_accuracies: List[List[float]], 
        ...
        for i, (accuracies, stats) in enumerate(zip(all_accuracies, all_matching_stats)):
            label = (f'Part {i+1} (Queries: {stats["total_queries"]}, '
                    f'Avg Matches: {stats["avg_matches"]:.1f})')
    ...
    def analyze_part(self, data_dir: str, video_text_map: Dict[str, str], 
        ...
        self.logger.info(f"Building text index for {part_name}...")
    ...
    def analyze_queries(self, data_dir: str, num_parts: int = 2):
        ...
        comparison_dir = os.path.join(data_dir, 'T5_2parts_analysis_results', 
